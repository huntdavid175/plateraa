...
- **Deployed API:** `https://plateraa-api.onrender.com` (Render, Frankfurt, free plan), deployed automatically from `main`. The free plan has no pre-deploy step, so **before pushing code that needs a new migration, run `pnpm --filter @plateraa/db db:migrate` (the laptop's development branch), then `db:migrate:production` (the live database)**.
- **Two databases** (Neon, Frankfurt): the live one (Render's, which the tablet uses) and a `dev` branch for the laptop and its tests. On the laptop, `DATABASE_URL` / `DATABASE_URL_DIRECT` are the dev branch; `PRODUCTION_DATABASE_URL` / `PRODUCTION_DATABASE_URL_DIRECT` are the live one, used only by `db:migrate:production` and by setup scripts run with `--production`. Tests refuse to run against the live database.
